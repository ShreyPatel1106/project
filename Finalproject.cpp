  ...
  while (playerHP > 0)
  {
    ...
    for (; EnemyHP > 0 && playerHP > 0;)
    {
      ...
      if (move == 'a')
      {
        ...
        if (rand() % 40 < 1)
          playerAtack = (Attack * 1.5); //40 percent chance f0r a 1.5x damage bonus
        else if (rand() % 10 < 1)
          playerAtack = (Attack * 2); //10 percent chance f0r a 2.0x damage bonus
        else
          playerAtack = Attack; //otherwise, normal hit
      }
      if (move == 'm')
      {

        if (rand() % 40 < 1)
          playerAtack = (Magic * 1.5); //40 percent chance f0r a 1.5x damage bonus
        else if (rand() % 10 < 1)
          playerAtack = (Magic * 2); //10 percent chance f0r a 2.0x damage bonus
        else
          playerAtack = Magic; //otherwise, normal hit
      }
...
void displayStats(string Name, string Class, string Race, string Perk, int Attack, int Defence, int Magic, int Stamina) // displays your stats and then puts their values into an array and sorts them
{
  ...
  cout << endl
       << "Stats in order from worse to best " << endl;
  for (int x = 0; x < 4; x++)
  {
...

{
  ...
  for (int i = 0; i != 4;) // Lets you create your chose 4 because that can never happen within this loop
  {
    if (i == 0)
    {
      ...
      if (input == 'n')
        i++; //"i" is increased by 1 if the choice is "N" , moving on to the next status screen when the loop runs again. "C" will close the loop. Anything else will run the loop again
      if (input == 'c')
      {
        Class = "Warrior";
        i = 4;
    ...
    if (i == 1)
    {
      ...
      Stamina = 5;
      if (input == 'p')
        i--; //if the user enters 'p', I is decreased by 1 and thus stops at the if statment before this one the next time the program runs through the loop
      if (input == 'n')
        i++; //does the same thing as before, i is increased to move on to the next choice
      if (input == 'c')
      {
        Class = "Mage";
        i = 4;
  ...
  for (int i = 0; i != 4;) // choose your perk (gives you stat advantages and disadvantages)
  {
    if (i == 0)
    {
      ...
      if (input == 'c')
      {
        Perk = "The best defense is a good offense";
        i = 4;
      }
    }
    if (i == 1)
